...
class Employee extends Model
{
   protected $fillable = [
     ...
     'el_remaining', 'status', 'nbi_clearance', 'annual_physical_exam'
   ];
    //
   ...
   {
     return $this->belongsToMany(Training::class)->withPivot('status');
   }

   public function storeUploadedTraining($file)
   {
     # code...
   }
   ...
   public static function adminUpdateEmployeeInformation($request, $employee)
   {
     ...
     if(!$request->hasFile('fileToUpload')) {
      ...
      $employee->update([
         ...
         'annual_physical_exam' => $request->get('annual_physical_exam')
      ]);
